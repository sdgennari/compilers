...
def gen_tac_for_binding(cur_tac_list, ast_binding):
...
def gen_tac_for_exp(cur_tac_list, ast_exp):
	...
	if isinstance(ast_exp, ASTExpString):
		...
		cond_symbol_boxed, cond_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.cond_exp)

		# Unbox condition variable
		cond_symbol_unboxed = gen_tac_for_unbox(cur_tac_list, cond_symbol_boxed, "Bool")
		...
		then_symbol, then_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.then_exp)
		...
		else_symbol, else_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.else_exp)
		...
		body_symbol, body_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.loop_exp)
		...
		exp_symbol, exp_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.exp)

		maybe_symbol = get_symbol(ast_exp.ident)
		
		# If the assignee symbol is None, then the ident is an attr
		if maybe_symbol == None:
			cur_tac_list.append(TACStoreAttr(exp_type_from_ast, ast_exp.ident, exp_symbol))
		else:
		...
		for block_exp in ast_exp.exp_list:
			last_assignee_symbol, last_type_from_ast = gen_tac_for_exp(cur_tac_list, block_exp)
		...
		if ast_exp.exp_type == "SELF_TYPE":
			...
			cur_tac_list.append(TACAlloc(ast_exp.type_from_ast, assignee_symbol, ast_exp.exp_type))
		...
		for ast_binding in ast_exp.binding_list:
			gen_tac_for_binding(cur_tac_list, ast_binding)

		# Handle expression after 'in'
		return_symbol, return_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.exp)
	...
	# ExpBinaryArith: (self, line, left_exp, right_exp)
		...
		lhs_symbol_boxed, lhs_symbol_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.left_exp)
		rhs_symbol_boxed, rhs_symbol_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.right_exp)

		# Unbox values
		lhs_symbol_unboxed = gen_tac_for_unbox(cur_tac_list, lhs_symbol_boxed, "Int")
		rhs_symbol_unboxed = gen_tac_for_unbox(cur_tac_list, rhs_symbol_boxed, "Int")
		...
		left_symbol, left_symbol_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.left_exp)
		right_symbol, right_symbol_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.right_exp)
	...
	# ExpUnary: (self, line, exp)
		...
		# Get the boxed value
		boxed_exp_symbol, boxed_exp_symbol_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_exp.exp)

		# Unbox the value and do 'not'
		unboxed_exp_symbol = gen_tac_for_unbox(cur_tac_list, boxed_exp_symbol, "Int")
		unboxed_assignee_symbol = new_symbol()
		...
		# Unbox the value and do 'not'
		unboxed_exp_symbol = gen_tac_for_unbox(cur_tac_list, boxed_exp_symbol, "Bool")
		unboxed_assignee_symbol = new_symbol()
		cur_tac_list.append(TACNegBool(ast_exp.type_from_ast, unboxed_assignee_symbol, unboxed_exp_symbol))

		# Box the answer and set assignee to pointer
		assignee_symbol = gen_tac_for_box(cur_tac_list, unboxed_assignee_symbol, "Bool")
		...
		for idx, exp in enumerate(ast_exp.exp_list):
			param_symbol, param_type_from_ast = gen_tac_for_exp(cur_tac_list, exp)
		...
		# 	cur_tac_list.append(TACOutString(ast_exp.type_from_ast, assignee_symbol, exp_symbol_list[0]))

		# elif ast_exp.ident == "out_int":
		# 	cur_tac_list.append(TACOutInt(ast_exp.type_from_ast, assignee_symbol, exp_symbol_list[0]))

		# elif ast_exp.ident == "in_string":
		# 	cur_tac_list.append(TACInString(ast_exp.type_from_ast, assignee_symbol))

		# elif ast_exp.ident == "in_int":
		# 	cur_tac_list.append(TACInInt(ast_exp.type_from_ast, assignee_symbol))
		...
		for ast_case_elem in ast_exp.case_exp_list:
			...
			case_elem_symbol = add_symbol(ast_case_elem.ident)
...
def gen_tac_for_feature(cur_tac_list, ast_feature, class_name):
	...
	if isinstance(ast_feature, ASTMethod):
		...
		return_symbol, return_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_feature.body_exp)
	...
	# 	exp_symbol, exp_type_from_ast = gen_tac_for_exp(cur_tac_list, ast_feature.exp)
	# 	cur_tac_list.append(TACAssign(ast_feature.feature_type, attr_symbol, exp_symbol))

	# elif isinstance(ast_feature, ASTAttrNoInit):
	# 	# AttrNoInit: (self, ident_line, ident, type_line, feature_type)
	# 	attr_symbol = add_symbol(ast_feature.ident)
	# 	cur_tac_list.append(TACDefault(ast_feature.feature_type, attr_symbol, ast_feature.feature_type))
...
# 	cur_tac_list.append(TACComment("start class: " + str(class_typ)))

# 	if class_typ != "Main":
# 		symbol_table_list.pop()
# 		cur_tac_list.append(TACComment("end class: " + str(class_typ)))
# 		return
...
# 	global symbol_table_list
# 	global tac_list
# 	cur_tac_list.append(TACComment("start"))
