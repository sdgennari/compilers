...
.Main_main_1:
			...
			## save self ptr (%rbx)
			pushq	%rbx
			## pushing 3 params to the stack
			subq	$24, %rsp
			## moving rsp[112] to rsp[0]
			movq	112(%rsp), %rax
			movq	%rax, 0(%rsp)
			## moving rsp[104] to rsp[8]
			movq	104(%rsp), %rax
			movq	%rax, 8(%rsp)
			## moving rsp[96] to rsp[16]
			movq	96(%rsp), %rax
			movq	%rax, 16(%rsp)
			...
			call	*%rax
			## removing 3 params from stack with subq
			addq	$24, %rsp
			...
			popq	%rdx
			popq	%rcx
			## removing 3 stored params from stack (2nd time)
			addq	$24, %rsp
