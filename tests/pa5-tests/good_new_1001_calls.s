...
.globl Chain..vtable
Chain..vtable:		## vtable for Chain
			.quad type_name_Chain
			.quad Chain..new
			.quad Object.abort
			.quad Object.copy
			.quad Object.type_name
...
.globl Chain..new
Chain..new:		## Constructor for Chain
			pushq	%rbp
			movq	%rsp, %rbp
			## allocate space to store 0 spilled regs
			subq	$0, %rsp
			...
			pushq	%r14
			pushq	%r15
			## allocate space for Chain (size: 4)
			movq	$8, %rsi
			movq	$4, %rdi
			call	calloc
			movq	%rax, %rbx
			...
			movq	$4, %rax
			movq	%rax, 8(%rbx)
			movq	$Chain..vtable, %rax
			movq	%rax, 16(%rbx)
			## default Chain
			movq	$0, %r8
			## store %r8 in self[3] (next)
			movq	%r8, 24(%rbx)
			## pop callee-saved regs
			popq	%r15
			popq	%r14
...
.dispatch_2_not_void:
			...
			movq	16(%r10), %rax
			## find method set_next in vtable[6]
			movq	48(%rax), %rax
...
.dispatch_6_not_void:
			...
			## get ptr to vtable from receiver obj
			movq	16(%r8), %rax
			## find method prepend in vtable[5]
			movq	40(%rax), %rax
			## call method dynamically
			call	*%rax
			## removing 1 params from stack with subq
...
.globl type_name_Chain 
type_name_Chain:			## type_name string for Chain
			.string "Chain"
