...
Main.main:
			pushq	%rbp
			movq	%rsp, %rbp
			## allocate space to store 6 spilled regs
			subq	$48, %rsp
			## push callee-saved regs
			pushq	%r12
			pushq	%r13
...
.Main_main_1:
			...
			popq	%rcx
			movq	%rax, %r9
			## check if %rdi is void and set result accordingly
			cmpq	$0, %rdi
			jnz		.asm_label_1
			movq	$1, 24(%r9)
.asm_label_1:
			## unbox value of %r9 into %rsi
			movq	24(%r9), %rsi
			## not
			movl	%esi, %r9d
			xorl	$1, %r9d
...
.dispatch_2_not_void:
			...
			movq	80(%rsp), %rax
			movq	%rax, 0(%rsp)
			## set receiver_obj (%rdi) as self ptr (%rbx)
			movq	%rdi, %rbx
			...
			popq	%rcx
			movq	%rax, %r9
			## check if %rsi is void and set result accordingly
			cmpq	$0, %rsi
			jnz		.asm_label_2
			movq	$1, 24(%r9)
.asm_label_2:
...
.dispatch_3_not_void:
			...
			movq	80(%rsp), %rax
			movq	%rax, 0(%rsp)
			## set receiver_obj (%rsi) as self ptr (%rbx)
			movq	%rsi, %rbx
			## dynamic: lookup method in vtable
			## get ptr to vtable from receiver obj
			movq	16(%rsi), %rax
...
.dispatch_5_not_void:
			...
			popq	%rcx
			movq	%rax, %r8
			## check if %r15 is void and set result accordingly
			cmpq	$0, %r15
			jnz		.asm_label_5
			movq	$1, 24(%r8)
.asm_label_5:
...
.dispatch_6_not_void:
			...
			movq	80(%rsp), %rax
			movq	%rax, 0(%rsp)
			## set receiver_obj (%r15) as self ptr (%rbx)
			movq	%r15, %rbx
			## dynamic: lookup method in vtable
			## get ptr to vtable from receiver obj
			movq	16(%r15), %rax
...
.asm_label_14:
			...
			movl	%r10d, %r8d
			xorl	$1, %r8d
			## branch .dispatch_15_void
			test	%r10d, %r10d
			jnz		.dispatch_15_void
			## branch .dispatch_15_not_void
			test	%r8d, %r8d
			jnz		.dispatch_15_not_void
.dispatch_15_void:
			movq	$string_15, %rdi
			call	raw_out_string
			movq	$0, %rax
			call	exit
			jmp		.dispatch_15_not_void
.dispatch_15_not_void:
			pushq	%rcx
			pushq	%rdx
			pushq	%rsi
			pushq	%rdi
...
.dispatch_17_not_void:
			...
			popq	%r13
			popq	%r12
			## remove temporary stack space for 6 spilled regs
			addq	$48, %rsp
			leave
			ret
...
.globl string_15
string_15:
			.string "ERROR: 34: Exception: dispatch on void"

.globl string_7
string_7:
			.string "ERROR: 26: Exception: dispatch on void"
...
.globl string_17
string_17:
			.string "ERROR: 36: Exception: dispatch on void"

.globl string_16
string_16:
			.string "ERROR: 35: Exception: dispatch on void"
...
.globl string_14
string_14:
			.string "ERROR: 33: Exception: dispatch on void"
...
.globl string_2
string_2:
			.string "ERROR: 21: Exception: dispatch on void"

.globl string_5
string_5:
			.string "ERROR: 24: Exception: dispatch on void"

.globl string_6
string_6:
			.string "ERROR: 25: Exception: dispatch on void"
...
.globl string_10
string_10:
			.string "ERROR: 29: Exception: dispatch on void"

.globl string_11
string_11:
			.string "ERROR: 30: Exception: dispatch on void"
