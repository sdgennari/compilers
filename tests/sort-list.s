...
Cons..new:		## Constructor for Cons
			...
			pushq	%r14
			pushq	%r15
			## allocate space for Cons (size: 5)
			movq	$8, %rsi
			movq	$5, %rdi
			call	calloc
			movq	%rax, %rbx
...
List..new:		## Constructor for List
			...
			pushq	%r14
			pushq	%r15
			## allocate space for List (size: 3)
			movq	$8, %rsi
			movq	$3, %rdi
			call	calloc
			movq	%rax, %rbx
...
.dispatch_6_not_void:
			...
			## get ptr to vtable from receiver obj
			movq	16(%r11), %rax
			## find method rcons in vtable[16]
			movq	128(%rax), %rax
			## call method dynamically
			call	*%rax
...
.dispatch_9_not_void:
			...
			## get ptr to vtable from receiver obj
			movq	16(%r9), %rax
			## find method insert in vtable[15]
			movq	120(%rax), %rax
			## call method dynamically
			call	*%rax
			## removing 1 params from stack with subq
...
.asm_label_5:
			...
			movl	%r10d, %r8d
			xorl	$1, %r8d
			## branch .dispatch_12_void
			test	%r10d, %r10d
			jnz		.dispatch_12_void
			## branch .dispatch_12_not_void
			test	%r8d, %r8d
			jnz		.dispatch_12_not_void
.dispatch_12_void:
			movq	$string_3, %rdi
			call	raw_out_string
			movq	$0, %rax
			call	exit
			jmp		.dispatch_12_not_void
.dispatch_12_not_void:
			pushq	%rcx
			pushq	%rdx
			pushq	%rsi
			pushq	%rdi
...
.asm_label_6:
			...
			movl	%r10d, %r8d
			xorl	$1, %r8d
			## branch .dispatch_13_void
			test	%r10d, %r10d
			jnz		.dispatch_13_void
			## branch .dispatch_13_not_void
			test	%r8d, %r8d
			jnz		.dispatch_13_not_void
.dispatch_13_void:
			movq	$string_4, %rdi
			call	raw_out_string
			movq	$0, %rax
			call	exit
			jmp		.dispatch_13_not_void
.dispatch_13_not_void:
			pushq	%rcx
			pushq	%rdx
			pushq	%rsi
			pushq	%rdi
...
.dispatch_16_void:
			...
			movq	$0, %rax
			call	exit
			jmp		.dispatch_16_not_void
.dispatch_16_not_void:
			pushq	%rcx
			pushq	%rdx
			pushq	%rsi
			pushq	%rdi
...
.dispatch_19_not_void:
			...
			## get ptr to vtable from receiver obj
			movq	16(%r9), %rax
			## find method print_list in vtable[17]
			movq	136(%rax), %rax
			## call method dynamically
			call	*%rax
			## removing 0 params from stack with subq
...
.dispatch_23_void:
			...
			movq	$0, %rax
			call	exit
			jmp		.dispatch_23_not_void
.dispatch_23_not_void:
			pushq	%rcx
			pushq	%rdx
			pushq	%rsi
			pushq	%rdi
...
.loop_start_32:
			...
			## assign
			movq	%r11, %r12
			## use lt_helper to compare %r10 < %r12
			## push caller-saved regs and self ptr
			pushq	%rcx
			pushq	%rdx
			pushq	%rsi
...
.Main_main_34:
			...
			## get ptr to vtable from self
			movq	16(%rbx), %rax
			## find method in_int in vtable[5]
			movq	40(%rax), %rax
			## call method dynamically
			call	*%rax
			## removing 0 params from stack with subq
...
.asm_label_14:
			...
			movl	%r9d, %r8d
			xorl	$1, %r8d
			## branch .dispatch_36_void
			test	%r9d, %r9d
			jnz		.dispatch_36_void
			## branch .dispatch_36_not_void
			test	%r8d, %r8d
			jnz		.dispatch_36_not_void
.dispatch_36_void:
			movq	$string_11, %rdi
			call	raw_out_string
			movq	$0, %rax
			call	exit
			jmp		.dispatch_36_not_void
.dispatch_36_not_void:
			pushq	%rcx
			pushq	%rdx
			pushq	%rsi
			pushq	%rdi
...
.globl type_name_Cons 
type_name_Cons:			## type_name string for Cons
			.string "Cons"
...
.globl type_name_List 
type_name_List:			## type_name string for List
			.string "List"
...
.globl type_name_Nil 
type_name_Nil:			## type_name string for Nil
			.string "Nil"
...
.globl string_11
string_11:
			.string "ERROR: 138: Exception: dispatch on void"

.globl string_3
string_3:
			.string "ERROR: 80: Exception: dispatch on void"

.globl string_12
string_12:
			.string "ERROR: 107: Exception: dispatch on void"
...
.globl string_5
string_5:
			.string "ERROR: 87: Exception: dispatch on void"

.globl string_4
string_4:
			.string "ERROR: 82: Exception: dispatch on void"

.globl string_8
string_8:
			.string "ERROR: 32: Exception: dispatch on void"

.globl string_1
string_1:
			.string "ERROR: 74: Exception: dispatch on void"
...
.globl string_9
string_9:
			.string "ERROR: 126: Exception: dispatch on void"
