...
Main.main:
			pushq	%rbp
			movq	%rsp, %rbp
			## allocate space to store 8 spilled regs
			subq	$64, %rsp
			## push callee-saved regs
			pushq	%r12
			pushq	%r13
...
.loop_start_2:
			...
			movq	%rax, %r8
			movq	%rsi, 24(%r8)
			## use lt_helper to compare %rdx < %r8
			...
			popq	%rdx
			popq	%rcx
			## move comparison result into %rsi
			movq	%rax, %rsi
			## unbox value of %rsi into %rdx
			movq	24(%rsi), %rdx
			## not
			movl	%edx, %r8d
			xorl	$1, %r8d
...
.loop_exit_2:
			...
			popq	%r13
			popq	%r12
			## remove temporary stack space for 8 spilled regs
			addq	$64, %rsp
			leave
			ret
